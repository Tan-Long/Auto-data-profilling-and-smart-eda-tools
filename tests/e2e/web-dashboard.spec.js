...
test("local path run renders the interactive dashboard from generated artifacts", async ({
  ...
  await expect(reportPage.locator("h2", { hasText: "Review briefing" })).toBeVisible();
  await expect(reportPage.locator("h2", { hasText: "Where are the problems?" })).toBeVisible();
  await expect(reportPage.getByText("DBML table map")).toBeVisible();
  ...
  await expect(reportPage.locator("h2", { hasText: "Fix / Verify lanes" })).toBeVisible();
  await expect(reportPage.getByText("Default fix").first()).toBeVisible();
  await expect(reportPage.getByText("OpenAI add-on").first()).toBeVisible();
