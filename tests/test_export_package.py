...
def test_package_output_directory_writes_manifest_index_and_zip(tmp_path):
    ...
    assert "Issue-first todo queue" in index_html
    assert "Issue todo queue" in index_html
    assert "Fix data snapshot" not in index_html
    assert "Verify after fix snapshot" not in index_html
    assert "Package index shows the first 10 todo groups per type" not in index_html
    assert "additional Fix data groups" not in index_html
    assert "additional Verify after fix groups" not in index_html
